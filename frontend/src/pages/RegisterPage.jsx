...
export default function RegisterPage() {
  ...
  const { loginWithGoogle, register, requestPhoneOtp, verifyPhoneOtp } = useAuth()
  ...
  const [phoneForm, setPhoneForm] = useState({ name: '', phone: '', otp: '' })
  ...
  const canSubmitEmail =
    !submitting &&
    Boolean(emailForm.name.trim()) &&
    Boolean(emailForm.email.trim()) &&
    Boolean(emailForm.password.trim())
  const canRequestOtp =
    !submitting && Boolean(phoneForm.name.trim()) && Boolean(phoneForm.phone.trim())
  ...
  function validateEmailForm() {
    ...
    if (emailForm.phone.trim() && !validatePhone(emailForm.phone)) {
      nextErrors.phone = 'Use a full phone number with country code, for example +15551234567.'
    }
      ...
      pushToast({
        ...
        message:
          response.delivery === 'sms'
            ? 'Enter the code sent to your phone to finish creating your account.'
    ...
    <div className="mx-auto w-full max-w-md px-4 py-10">
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-card md:p-8">
        ...
        <p className="mt-2 text-sm text-slate-300">
          Save delivery details, track purchases, and keep your favorites in one place.
        </p>
        ...
        {method === 'email' ? (
          <form onSubmit={onEmailSubmit} className="mt-6 grid gap-4">
            ...
            <Input
              ...
              hint="Add your number now if you want phone-based sign-in later."
            />
            <Input
              label="Password"
              name="password"
          ...
          >
            ...
            <Input
              ...
              placeholder="+15551234567"
              hint="We'll send a verification code to finish your sign-up."
              disabled={phoneStep === 'verify'}
            />
